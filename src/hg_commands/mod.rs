...
pub fn hg_pull(repo_list: &Option<Vec<String>>) {
    let repo_list = repo_list.clone();
    thread::spawn(move || match repo_list {
        Some(repo_list) => {
            let os_arg = if cfg!(target_os = "windows") {
            ...
            for repo in repo_list {
                ...
                let output = cmd
                    .arg(os_arg)
                    .arg("cd /D ".to_owned() + &repo + " & hg pull")
                    .output()
                    .expect("failed to execute command");
...
pub fn hg_push(repo_list: &Option<Vec<String>>) {
    let repo_list = repo_list.clone();
    thread::spawn(move || match repo_list {
        Some(repo_list) => {
            let os_arg = if cfg!(target_os = "windows") {
            ...
            for repo in repo_list {
                ...
                let output = cmd
                    .arg(os_arg)
                    .arg("cd /D ".to_owned() + &repo + " & hg push")
                    .output()
                    .expect("failed to execute command");
...
pub fn hg_switch(repo_list: &Option<Vec<String>>) {}

pub fn hg_purge(repo_list: &Option<Vec<String>>) {
    let repo_list = repo_list.clone();
    thread::spawn(move || match repo_list {
        Some(repo_list) => {
            let os_arg = if cfg!(target_os = "windows") {
            ...
            for repo in repo_list {
                ...
                let output = cmd
                    .arg(os_arg)
                    .arg("cd /D ".to_owned() + &repo + " & hg purge")
                    .output()
                    .expect("failed to execute command");
